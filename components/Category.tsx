...
export default function Category({name,color}: Props) {

  return (
    <Link href={`/${name}`}>
    <div className={`p-2 w-60 h-20 m-5 text-white items-center flex justify-between
    overflow-hidden cursor-pointer
    rounded-lg shadow-2xl shadow-yellow-400 ${color ? "bg-yellow-400 shadow-yellow-400" :
    "bg-cyan-500 shadow-cyan-500 "}`}
    >
       <p className='font-bold'> {name}</p>
       <svg id="Layer_1" 
       className={`${color ? "fill-yellow-300" :
    "fill-cyan-400 "}  w-[60%]`}
       data-name="Layer 1" 
